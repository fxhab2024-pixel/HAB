...
# Caddy is already managed by the platform (PID 2, /app/Caddyfile).
# Do NOT try to start Caddy here - it will fail because port 81 is already bound.
echo "ℹ️  Skipping Caddy startup (managed by platform on port 81)"
echo ""
echo "🎉 Next.js service is ready!"
echo ""
echo "💡 Caddy (PID 2) proxies port 81 → localhost:3000"
echo ""

# Keep the script running so the deploy endpoint knows the service started successfully
# Wait for Next.js to be ready
sleep 2
if curl -s --connect-timeout 2 --max-time 5 "http://localhost:3000" >/dev/null 2>&1; then
    echo "✅ Next.js health check passed - server is responding on port 3000"
else
    echo "⚠️  Next.js may not be ready yet on port 3000"
fi
